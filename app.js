...
function defaultState(){
  const w={id:uid(),name:'バイト先',hourlyWage:1000};
  const p={id:uid(),workplaceId:w.id,name:'夕方',start:'17:00',end:'21:00',breakMinutes:0};
  return {version:4,settings:{monthlyGoal:100000,goalPresetId:p.id},workplaces:[w],presets:[p],shifts:[],fixedCosts:[]};
}
function normalize(s){
  const b=defaultState();
  const o={...b,...s,version:4,settings:{...b.settings,...(s?.settings||{})}};
  o.workplaces=Array.isArray(o.workplaces)&&o.workplaces.length?o.workplaces.map(w=>({id:w.id||uid(),name:w.name||'バイト先',hourlyWage:Math.max(0,num(w.hourlyWage??w.wage))})):b.workplaces;
...
function migrateV3(v){
  ...
  const workplaces=(v.workplaces||[]).map(w=>{
    ...
    return {id:w.id||uid(),name:w.name||'バイト先',hourlyWage:num(ws.at(-1)?.hourlyWage??w.hourlyWage)};
  });
  ...
  return normalize({version:4,settings:{monthlyGoal:num(v.settings?.salaryGoal??v.settings?.monthlyGoal,100000),goalPresetId:v.settings?.goalPresetId||''},workplaces:workplaces.length?workplaces:undefined,presets,shifts,fixedCosts});
}
function load(){
  try{
...
function renderSettings(){
  ...
  $('workplaceList').innerHTML=state.workplaces.map(w=>`<div class="quiet-row"><div class="quiet-main"><b>${esc(w.name)}</b><small>時給 ${yen(w.hourlyWage)}</small></div><div class="quiet-side"><div class="row-actions"><button class="mini-link" data-edit-workplace="${w.id}">編集</button><button class="mini-link" data-delete-workplace="${w.id}">削除</button></div></div></div>`).join('');
...
function openWorkplaceEditor(id=''){
  const w=state.workplaces.find(x=>x.id===id);$('workplaceDialogTitle').textContent=w?'バイト先を編集':'バイト先を追加';$('workplaceId').value=w?.id||'';$('workplaceName').value=w?.name||'';$('workplaceWage').value=w?.hourlyWage??'';openDialog('workplaceDialog')
}
...
$('workplaceForm').onsubmit=e=>{e.preventDefault();const id=$('workplaceId').value||uid(),item={id,name:$('workplaceName').value.trim(),hourlyWage:Math.max(0,num($('workplaceWage').value))},i=state.workplaces.findIndex(x=>x.id===id);if(i>=0)state.workplaces[i]=item;else state.workplaces.push(item);save();closeDialogs();toast('保存しました')};
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=4.0.0').catch(console.error));
renderAll();
